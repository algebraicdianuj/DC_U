...
def offline_trainer(new_directory_path,net,img_real_data_loader, img_syn_loader, retain_loader, forget_loader, device, num_classes, offline_condensation_iterations, final_model_epochs, databank_model_epochs, lr_final, lr_databank,channel,im_size,hidden_size):

    beggining=Beginning(input_size=channel * im_size[0] * im_size[1], hidden_size=hidden_size).to(device)
    beggining.fc1.weight.data = net.fc1.weight.data
    beggining.fc1.bias.data = net.fc1.bias.data

    intermediate=Intermediate(hidden_size=hidden_size).to(device)
