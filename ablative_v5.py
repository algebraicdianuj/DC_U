...
def main(args):
    ...
    #=========================== Main Change=====================================================================
    feature_extractor = get_model(args.feature_model_name, args.exp, data_storage, num_classes, device, load=False)
    uniq_residual_labels, uniq_residual_counts = torch.unique(residual_labels, return_counts=True)
    print(f"Unique Labels in Residual Set: {uniq_residual_labels} with counts {uniq_residual_counts}")
    print(f'I am going to target num_subclasses: {n_subclasses} for each class')
    del uniq_residual_labels, uniq_residual_counts
    ...
    target_images, target_labels, sub_labels = condensation_sampling(indices_retain_wrt_finelabels, residual_images, residual_labels)
    ...
    save_data(result_directory_path, 'L1_sparsity_ablv5', df, args.model_name, args.exp, choice)
