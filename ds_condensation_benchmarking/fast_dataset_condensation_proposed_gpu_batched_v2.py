...
def main():
    ...
    print('Running GPU batched Version of FDM-version 2...\n\n')
    ...
    class WeightedAverage(nn.Module):
        def __init__(self, num_images):
            ...
            self.weights = nn.Parameter(1/num_images*torch.ones(num_images, device=device))
    ...
    class BatchWeightedAverage(nn.Module):
        def __init__(self, batch_size, num_images_per_batch, bsizes, channels, height, width,device=device):
            ...
            # for i, bsize in enumerate(bsizes):
            #     if bsize < num_images_per_batch:
            #         self.weights.data[i] *= 1/bsize

            # # Normalize the weights for each batch
            # for i in range(batch_size):
            #     non_zero_sum = self.weights.data[i].sum()
            #     if non_zero_sum > 0:
            #         self.weights.data[i] /= non_zero_sum
    ...
    def Average(ref_imgs_all_batched, bsizes, pretrained=net, channels = 3, height = 32, width = 32, num_epochs=100):
    ...
    for exp in range(len(number_subclasses)):
        ...
        for idx in bucket_labbies:
            ...
            if len(collected_ref_imgs_all)==batcher or idx==bucket_labbies[-1]:
                collected_ref_imgs_all,bsizes=correct_sizes(collected_ref_imgs_all, channel, im_size[0], im_size[1])
                ref_imgs_all_batched=torch.stack(collected_ref_imgs_all)

                inverted_image_batch = Average(ref_imgs_all_batched, bsizes, pretrained=net, num_epochs=100)
    ...
    df.to_csv('benchmark_proposed_fdm_gpu_batched_v2.csv', index=False)
