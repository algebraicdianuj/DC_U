...
from modular_unlearn.offline_training_resnet import *
...
def main():
    ...
    #----------------------------Loading stuff------------------------------------------------------------------------
    resmodel = resnet18(pretrained=False, num_classes=num_classes)
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    net=Resnet18(resmodel).to(device)
    ...
    file_path = os.path.join(result_directory_path,'resnet_modular_unlearning_stats.csv')
    ...
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    naive_net=Resnet18(resmodel).to(device)
    starting_time = time.time()
    ...
    file_path = os.path.join(result_directory_path,'resnet_retraining_unlearning_stats.csv')
    df.to_csv(file_path, index=False)
    ...
    file_path = os.path.join(result_directory_path,'resnet_catastrophic_unlearning_stats.csv')
    df.to_csv(file_path, index=False)
    ...
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    teacher_net=Resnet18(resmodel).to(device)
    file_path = os.path.join(new_directory_path,'pretrained_net.pth')
    teacher_net.load_state_dict(torch.load(file_path))
    resmodel = resnet18(pretrained=False, num_classes=num_classes)
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    student_net=Resnet18(resmodel).to(device)
    starting_time = time.time()
    ...
    file_path = os.path.join(result_directory_path,'resnet_distillation_unlearning_stats.csv')
    df.to_csv(file_path, index=False)
    ...
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    bad_teacher_net=Resnet18(resmodel).to(device)
    starting_time = time.time()
    ...
    file_path = os.path.join(result_directory_path,'resnet_bad_teacher_distillation_unlearning_stats.csv')
    df.to_csv(file_path , index=False)
    ...
    file_path = os.path.join(result_directory_path,'resnet_sparsification_unlearning_stats.csv')
    df.to_csv(file_path, index=False)



    #-----------------------Sparsification-Pruning based Method---------------------------------------------------------------------------------
    resmodel = resnet18(pretrained=False, num_classes=num_classes)
    resmodel.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    resmodel.maxpool = nn.Identity()
    naive_net=Resnet18(resmodel).to(device)
    ...
    file_path = os.path.join(result_directory_path,'resnet_prunning_unlearning_stats.csv')
    df.to_csv(file_path, index=False)
