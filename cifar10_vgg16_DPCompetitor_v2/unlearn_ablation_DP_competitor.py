...
def main():
    ...
    #----------------------------Loading stuff------------------------------------------------------------------------
    vgg=modify_vgg16(channel, im_size[0], num_classes)
    net=Vgg16(vgg16=vgg).to(device)
    ...
    del combined_model_copy
    del combined_model
    del ref_net
    #--------------------------------DP training---------------------------------------------------------------------------------------------
    random_classes=[3,4,6,2,8,0]
    trainloader=torch.utils.data.DataLoader(img_real_data_dataset, batch_size=batch_sgd, shuffle=True)
    vgg=modified_vgg16(channel, im_size[0], num_classes)
    net=vgg.to(device)
    starting_time = time.time()
    # dp_net=DP_Adam(net=net,train_loader=img_real_data_loader,max_grad_norm=MAX_GRAD_NORM, epsilon=EPSILON, delta=DELTA, multiplier=MULTIPLIER, train_lr=retrain_lr, train_epochs=2, device=device)
    dp_net=DP_ADAM(net=net,train_loader=trainloader,max_grad_norm=MAX_GRAD_NORM, epsilon=EPSILON, delta=DELTA, multiplier=MULTIPLIER, train_lr=sgd_lr, train_epochs=70, device=device)
    ending_time = time.time()
    training_time=ending_time - starting_time
    print('DP training time: ', training_time)
    ...
    plt.plot([0, 1], [0, 1], color='gray', linestyle='--', label='45-degree Reference')
