...
def main():
    ...
    for cycle in range(num_cycles):
        if cycle==0:
            ...
            future_retain_indices=retain_indices.clone()
            ...
            print("Cycle: ", cycle)
            print("Ratio of forget to retain: ", len(forget_set_real)/len(retain_set_real))
            print("-------------------------------------------")
        else:
            split = int(split_ratio * len(future_retain_indices))

            # Split indices into two parts
            new_forget_indices = future_retain_indices[:split]
            new_retain_indices = future_retain_indices[split:]

            # add new forget indices to the old forget indices
            new_forget_indices = torch.cat((forget_indices, new_forget_indices), dim=0)

            new_forget_images=train_images[new_forget_indices]
            new_forget_labels=train_labels[new_forget_indices]

            new_retain_images=train_images[new_retain_indices]
            new_retain_labels=train_labels[new_retain_indices]

            new_forget_set_real=TensorDatasett(new_forget_images, new_forget_labels)
            new_retain_set_real=TensorDatasett(new_retain_images, new_retain_labels)

            new_forget_loader=torch.utils.data.DataLoader(new_forget_set_real, batch_size=batch_size, shuffle=True)
            new_retain_loader=torch.utils.data.DataLoader(new_retain_set_real, batch_size=batch_size, shuffle=True)

            reduced_imgs,reduced_labs= offline_data_processing(inverted_IMG, inverted_LABEL, indices_train_wrt_bucket, new_forget_indices, new_retain_indices, bucket_labbies, img_real_data_dataset,train_images,train_labels,new_forget_loader, new_retain_loader,device)
            future_retain_indices=new_retain_indices.clone()
            file_path = os.path.join(new_directory_path,'reduced_retain_images'+str(cycle)+'.pth')
            torch.save(reduced_imgs, file_path)
            file_path = os.path.join(new_directory_path,'reduced_retain_labels'+str(cycle)+'.pth')
            torch.save(reduced_labs, file_path)
            file_path = os.path.join(new_directory_path,'forget_set'+str(cycle)+'.pth')
            torch.save(new_forget_set_real, file_path)
            file_path = os.path.join(new_directory_path,'retain_set'+str(cycle)+'.pth')
            torch.save(new_retain_set_real, file_path)
            print("Cycle: ", cycle)
            print("Ratio of forget to retain: ", len(new_forget_set_real)/len(new_retain_set_real))
            print("-------------------------------------------")
