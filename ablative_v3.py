...
def main(args):
    ...
    #=========================== Main Change=====================================================================
    feature_extractor = get_model(args.feature_model_name, args.exp, data_storage, num_classes, device, load=False)
    uniq_free_labels, uniq_free_counts = torch.unique(free_labels, return_counts=True)
    print(f"Unique Labels in Free Set: {uniq_free_labels} with counts {uniq_free_counts}")
    print(f'I am going to target num_subclasses: {n_subclasses} for each class')
    del uniq_free_labels, uniq_free_counts
    ...
    target_images, target_labels, sub_labels = condensation_sampling(indices_retain_wrt_finelabels, free_images, free_labels)
    ...
    save_data(result_directory_path, 'L1_sparsity_ablv3', df, args.model_name, args.exp, choice)
