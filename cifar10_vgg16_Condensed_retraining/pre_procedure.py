...
def main():
    ...
    for epochy in range(training_epochs):
        for batch in train_loader:
            ...
            output_cl = net.super_forward(data) 
            output_auto=net(data)
            loss_cl= nn.CrossEntropyLoss()(output_cl, label)
            loss_auto= nn.MSELoss()(output_auto, data)
            total_loss=loss_cl+0.5*loss_auto
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()

        print('Epoch: {} \tLoss: {:.6f}'.format(epochy+1, total_loss.item()))

        if epochy%10==0 or epochy==training_epochs-1 or epochy==0:
            ...
            with torch.no_grad():
                for data in train_loader:
                    ...
                    outputs = net.super_forward(images)
                    _, predicted = torch.max(outputs.data, 1)
                    total += labels.size(0)
                    correct += (predicted == labels).sum().item()
