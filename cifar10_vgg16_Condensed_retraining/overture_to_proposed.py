...
def main():
    ...
    # # Adjusting the input features of the classifier based on the image size
    # classifier_input_size = 512 * (im_size[0] // 32) ** 2 
    ...
    offline_trainer(new_directory_path=new_directory_path, net=net, img_real_data_loader=img_real_data_loader, img_syn_loader=img_syn_loader, 
                    retain_loader=retain_loader, forget_loader=forget_loader, device=device, offline_condensation_iterations=offline_condensation_iterations, final_model_epochs=final_model_epochs, 
                    databank_model_epochs=databank_model_epochs, lr_final=lr_final, lr_databank=lr_databank, channel=channel, im_size=im_size, num_classes=num_classes)
    #-------------------------------------------------------------------------------------------------------------------
