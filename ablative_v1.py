...
from utils.dc_utils import TensorDataset
...
def main(args):
    ...
    #=========================== Main Change=====================================================================
    feature_extractor = get_model(args.feature_model_name, args.exp, data_storage, num_classes, device, load=False)
    uniq_r_labels, uniq_r_counts = torch.unique(r_labels, return_counts=True)
    print(f"Unique Labels in Retain Set: {uniq_r_labels} with counts {uniq_r_counts}")
    print(f'I am going to target num_subclasses: {n_subclasses} for each class')
    del uniq_r_labels, uniq_r_counts
    _ , indices_retain_wrt_finelabels, _ = create_sub_classes(
                                                            r_images, 
                                                            r_labels, 
                                                            model=feature_extractor, 
                                                            num_classes=num_classes, 
                                                            sub_divisions=n_subclasses, 
                                                            device=device)
    print(f"Total Sub Classes: {len(indices_retain_wrt_finelabels.keys())}")
    nonempty_subclasses = 0
    for key, value in indices_retain_wrt_finelabels.items():
        if len(value) > 0:
            nonempty_subclasses += 1
    print(f"Non-empty Sub Classes: {nonempty_subclasses}")
    print(f"Classes: {num_classes}")
    print(f"Subclasses: {n_subclasses}")

    del feature_extractor
    
    print(f"Retain Set Images Shape: {r_images.shape}")
    print(f"Retain Set Labels Shape: {r_labels.shape}")
    target_images, target_labels, sub_labels = condensation_sampling(indices_retain_wrt_finelabels, r_images, r_labels)
    ...
    save_data(result_directory_path, 'L1_sparsity_ablv1', df, args.model_name, args.exp, choice)
